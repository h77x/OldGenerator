...
public final class LimitedRegionBlockAccess implements BlockAccess {
    ...
    public void setType(final int x, final int y, final int z, final Material material) {
        if (this.isInRegion(x, y, z)) this.region.setType(x, y, z, material);
    }
    ...
    public BlockData getBlockData(final int x, final int y, final int z) {
        return this.isInRegion(x, y, z) ? this.region.getBlockData(x, y, z) : Material.AIR.createBlockData();
    }

    @Override
    public void setBlockData(final int x, final int y, final int z, final BlockData data) {
        if (this.isInRegion(x, y, z)) this.region.setBlockData(x, y, z, data);
    }
    ...
    public int getHighestBlockYAt(final int x, final int z) {
        return this.isInRegion(x, this.minHeight, z)
                ? this.region.getHighestBlockYAt(x, z)
                : this.minHeight;
    }

    @Override
    public int getHighestBlockYAt(final int x, final int z, final HeightMap map) {
        return this.isInRegion(x, this.minHeight, z)
                ? this.region.getHighestBlockYAt(x, z, map)
                : this.minHeight;
    }
